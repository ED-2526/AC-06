...
# NOU: Necessari per al mode Kaggle (Content Based avançat)
...
DATASETS = {
    ...
    "kaggle": {
        ...
        "ratings_file": "ratings.csv",
        "keywords_file": "keywords.csv", # Només per Kaggle
        "type": "json"
    }
}
...
def parse_json_column(x):
    """Transforma JSON string a string separat per pipes (|). Serveix per Genres i Keywords."""
    try:
...
def load_datasets(dataset_key):
    ...
    # Lògica específica per Kaggle (JSONs i Keywords)
    if conf["type"] == "json":
        ...
        # Càrrega de Keywords (només si és Kaggle i està configurat)
        k_file = conf.get("keywords_file")
        if k_file:
            k_path = os.path.join(base_path, k_file)
            if os.path.exists(k_path):
                print("   ... carregant keywords...")
                try:
                    kw_df = pd.read_csv(k_path)
                    kw_df['id'] = pd.to_numeric(kw_df['id'], errors='coerce')
                    kw_df.dropna(subset=['id'], inplace=True)
                    kw_df['id'] = kw_df['id'].astype(int)
                    kw_df['keywords'] = kw_df['keywords'].apply(parse_json_column)
                    
                    # Fusionem keywords dins de movies
                    movies = movies.merge(kw_df, left_on='movieId', right_on='id', how='left')
                    if 'id' in movies.columns: movies.drop(columns=['id'], inplace=True)
    ...
    # Lògica Small
        ...
        movies['keywords'] = "" # Buit per defecte
    ...
    print(f"✅ Dades carregades. Movies: {len(movies)} | Ratings: {len(ratings)}")
...
def build_content_matrix(movies, dataset_type):
    """
    Construeix la matriu de contingut.
    - Si és 'kaggle': Usa TF-IDF amb Gèneres i Keywords.
    - Si és 'small': Usa matriu binària de Gèneres (classic).
    """
    if dataset_type == 'kaggle':
        print("   ... construint matriu de contingut AVANÇADA (TF-IDF: Gèneres + Keywords)...")
        # Unim generes i keywords
        soup = movies['genres'].str.replace('|', ' ', regex=False).fillna('') + " " + \
               movies['keywords'].str.replace('|', ' ', regex=False).fillna('')
        
        # Usem TF-IDF limitat a 500 features per no saturar memòria
        ...
        print(f"      -> TF-IDF creat amb {content_matrix.shape[1]} features.")
        return content_matrix

    else:
        print("   ... construint matriu de contingut BÀSICA (Només Gèneres)...")
        all_genres = set()
        for gen_str in movies["genres"].fillna(""):
            for g in str(gen_str).split("|"):
                if g and g != "(no genres listed)": all_genres.add(g)
        
        content_matrix = pd.DataFrame(0, index=movies["movieId"], columns=sorted(all_genres), dtype=int)
        for _, row in movies.iterrows():
            m_id = int(row["movieId"])
            for g in str(row["genres"]).split("|"):
                if g in content_matrix.columns: content_matrix.at[m_id, g] = 1
        
        return content_matrix

def build_user_profiles(train, content_matrix):
    profiles = {}
    grouped = train.groupby("userId")
    # Filtrem pelis valides
    valid_movies = set(content_matrix.index)

    for user, group in grouped:
        profile = np.zeros(content_matrix.shape[1], dtype=float)
        for _, r in group.iterrows():
            m_id = int(r["movieId"])
            if m_id in valid_movies:
                # Pes = Rating * Vector Pel·lícula
                profile += float(r["rating"]) * content_matrix.loc[m_id].values
        if np.linalg.norm(profile) > 0: profiles[user] = profile
    return profiles

def predict_content_based(user, item, user_profiles, content_matrix, user_means):
    base = user_means.get(user, 3.0)
    if item not in content_matrix.index or user not in user_profiles: return base
    
    user_vec = user_profiles[user]
    movie_vec = content_matrix.loc[item].values
    
    num = np.dot(user_vec, movie_vec)
    den = np.linalg.norm(user_vec) * np.linalg.norm(movie_vec)
    
    if den == 0: pred = base
    else: pred = base + (num/den) * (5.0 - base)
...
def predict_single_rating(user, item, mode_alg, mode_sim, data, k=10):
    matrix, user_means = data["matrix"], data["user_means"]

    if mode_alg == 'content':
        return predict_content_based(user, item, data["user_profiles"], data["content_matrix"], user_means)
    elif mode_alg == 'funk':
        return predict_funk_svd(user, item, data["funk_model"], user_means)
...
def evaluate_model(data, mode, k=10, override_model=None):
    """
...
def compute_all_data(dataset_key, k_filter):
    ...
    print("⚙️  Entrenant Funk SVD (Default factors=20)...")
    funk_model = train_funk_svd(matrix, n_epochs=15, verbose=False)

    # Construcció condicionada del contingut
    content_matrix = build_content_matrix(movies, dataset_key)
    user_profiles = build_user_profiles(train, content_matrix)
    
    return {
        "dataset_name": dataset_key, # Important per als ifs
        ...
        "sim_user_df_manual": None, "sim_item_df_manual": None,
        "funk_model": funk_model,
        "content_matrix": content_matrix, # Passem la matriu de contingut
...
def main_menu():
    ...
    while True:
        st = f"{current_dataset}" if data else "Cap"
        print(f"\n=== RECOMANADOR v19 (Loaded: {st}) ===")
        print("1. Carregar Dataset (Small / Kaggle)")
        print("2. Predir Valoració")
        print("3. Generar Gràfics i Mètriques")
        print("4. Sortir")
        
        op = input("Opció: ")

        if op == "1":
            ds = input("Dataset (s=Small, k=Kaggle): ").strip().lower()
            key = "kaggle" if ds == 'k' else "small"
            
            # Default K adaptatiu
            default_k = 100 if key == 'kaggle' else 5
            try: k_val = int(input(f"Filtre K (min vots) [Default={default_k}]: ") or default_k)
            except: k_val = default_k
            
            fname = get_cache_filename(key, k_val)
            if os.path.exists(fname):
                print("✅ Carregant cache..."); 
            ...
            if data: current_dataset = key

        elif op == "2" and data:
            try:
                u = int(input("User ID: "))
                m = int(input("Movie ID: "))
                alg = input("Algorisme (user/item/content/funk): ")
                p = predict_single_rating(u, m, alg, data)
            ...
            print("Opció incorrecta o dades no carregades.")
