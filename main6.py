...
# ---------------------------------------------------------
# 10. MENÚ I INTERACCIÓ
# ---------------------------------------------------------

def input_user_id(matrix):
    """Demana i valida un User ID"""
    while True:
        try:
            ...
            if uid in matrix.index:
                ...
                print(f"❌ L'usuari {uid} no existeix (ha estat filtrat o no hi és). Prova'n un altre.")
        except ValueError:
            print("⚠️ Si us plau, introdueix un número enter vàlid.")


def input_movie_id(movies_df):
    """Demana i valida un Movie ID"""
    while True:
        try:
            ...
            if mid in movies_df["movieId"].values:
                ...
                print(f"❌ La pel·lícula {mid} no existeix a la base de dades.")
        except ValueError:
            print("⚠️ Si us plau, introdueix un número enter vàlid.")


def select_algorithm():
    """Menú per seleccionar l'algorisme"""
    while True:
        print("\n   [ Algorisme ]")
...
def main_menu():
    path = "./ml_latest_small"  # Assegura't que aquesta carpeta existeix amb els csv

    if not os.path.exists(path):
        print(f"❌ Error: No s'ha trobat el directori '{path}'. Revisa la ruta.")
        return

    print("⏳ Carregant i processant dades, espera un moment...")
    ...
    print(f"✅ Dades llestes! Usuaris: {matrix.shape[0]}, Pel·lícules: {matrix.shape[1]}")

    while True:
        print("\n" + "=" * 40)
        print("       SISTEMA DE RECOMANACIÓ")
        print("=" * 40)
        print("1. Predir una valoració (Rating prediction)")
        print("2. Suggerir rànquing de pel·lícules (Top-N)")
        print("3. Avaluar model (RMSE Metrics)")
        print("4. Sortir")
        print("-" * 40)
        ...
        if opcio == "1":
            ...
            # Busquem títol per fer-ho bonic
            titol = movies[movies.movieId == movie_id]['title'].values[0]
            print(f"\n🔮 Calculant predicció per '{titol}'...")
            ...
            try:
                ...
                print(f"⭐ Predicció ({algo}): {pred:.4f}")
            except Exception as e:
                print(f"❌ Error durant el càlcul: {e}")
            ...
            print(f"\n🎬 TOP {n} RECOMANACIONS PER USUARI {user_id} ({algo.upper()}):")
            for i, (mid, title, score) in enumerate(recs, 1):
                print(f"{i}. [{score:.2f}] {title} (ID: {mid})")

        elif opcio == "3":
            print("\n📊 AVALUANT MODELS (Això pot trigar una mica)...")

            print("   -> Calculant RMSE User-User...")
            rmse_u = evaluate(test, matrix, mode="user", user_means=user_means)
            print(f"      RMSE User-User: {rmse_u:.4f}")

            print("   -> Calculant RMSE Item-Item...")
            rmse_i = evaluate(test, matrix, mode="item", user_means=user_means)
            print(f"      RMSE Item-Item: {rmse_i:.4f}")

            print("   -> Calculant RMSE Content-Based...")
            rmse_c = evaluate_content_based(movies, train, test)
            print(f"      RMSE Content-Based: {rmse_c:.4f}")
