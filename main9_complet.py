...
def generate_metrics_and_plots(movies, train, test, matrix, user_means, sim_user_df_fast, sim_item_df_fast,
    ...
    # Entrenem un SVD específic per l'avaluació (és sempre el mateix, no es compara manual vs fast)
    ...
    # Avaluacions MANUAL (si existeixen i estan en cache)
    ...
    if comp_names:
        ...
        # --- 2. Sweep de k (veïns) per FAST i MANUAL ---
        print("\n   -> Sweep de k (veïns) per User-User i Item-Item (mode Fast i Manual)...")
        k_list = [5, 10, 20, 40, 60, 80, 120, 160]
    ...
    # 3. Sweep de factors Funk-SVD
    print("\n   -> Sweep de nombre de factors per Funk-SVD...")
    factors_list = [5, 10, 20, 40]
    rmse_funk_list = []
...
def select_algorithm(ask_for_sim_mode=False):
    while True:
        ...
        if alg_choice in ['u', 'i', 'c', 'f']:
            ...
            # Només preguntem pel mode de similitud si és col·laboratiu
...
def compute_all_data(base_path):
    ...
    # 2. Entrenament Funk SVD (Model per defecte per a les prediccions)
...
def main_menu():
    ...
    # Desempaquetem per us ràpid (opcional, podem usar data directament)
    matrix = data["matrix"]
    movies = data["movies"]

    while True:
        print("\n" + "=" * 45)
        print(" SISTEMA DE RECOMANACIÓ - COMPLET (v3.0)")
        print("=" * 45)
        ...
        if op == "1":
            ...
            try:
                ...
                mode_alg, mode_sim = select_algorithm(ask_for_sim_mode=True)
                try:
                    n = int(input("   Quantes pel·lícules? (10): ") or 10)
                except:
                    n = 10
            ...
            # Crida a la nova funció d'avaluació completa
