...
# Alias: mateix parser per a 'keywords' (JSON style)
...
def load_datasets(dataset_key):
    ...
    if conf["type"] == "json":
        ...
        # (Només Kaggle) carregar keywords.csv si existeix i afegir-les a movies
    ...
    # Assegurem columna 'keywords' (per compatibilitat)
...
def round_rating_to_nearest_half(x):
    """Arrodoneix a l'enter o al 0.5 més proper (ex: 1.8 -> 2.0, 1.6 -> 1.5)."""
    try:
...
def build_tfidf_movie_matrix(movies, use_keywords=True):
    """Construeix una matriu TF-IDF per pel·lícula a partir de gèneres (+ keywords si cal)."""
...
def build_user_profiles_tfidf(train, tfidf_matrix, movieid_to_row):
    """Perfil d'usuari com a suma ponderada (per rating) de vectors TF-IDF de pel·lícules vistes."""
    profiles = {}
    grouped = train.groupby("userId")
    for user, group in grouped:
        profile_vec = None
        ...
        if profile_vec is not None and profile_vec.nnz > 0:
            # Normalitzem a norm-1 per estabilitat (opcional però útil)
...
def predict_single_rating(user, item, mode_alg, mode_sim, data, k=10):
    ...
    if mode_alg == 'content':
        # Content-Based: Small -> només gèneres | Kaggle -> gèneres + keywords (TF-IDF)
...
def evaluate_model(data, mode, k=10, override_model=None, override_sim_df=None):
    ...
    if mode == 'user':
        ...
        # Small -> només gèneres | Kaggle -> gèneres + keywords (TF-IDF) si està disponible
        if data.get('dataset_name') == 'kaggle' and data.get('tfidf_matrix') is not None and data.get('user_profiles_tfidf') is not None:
...
def plot_rmse_vs_k_neighbors(data):
    ...
    if current_dataset == 'kaggle':
        k_list = [20, 50, 200, 500, 800, 1200, 1500]
    else:
...
def plot_rmse_vs_factors_svd(data):
    ...
    current_dataset = data.get("dataset_name", "small")
    if current_dataset == 'kaggle':
        factors = [50, 100, 200, 300, 400, 800, 1200]
    else:
    ...
    for f in factors:
        print(f"      ... n_factors={f}")
        temp_model = train_funk_svd(matrix, n_factors=f, n_epochs=100, lr=0.01, reg=0.02, verbose=False)
    ...
    if current_dataset == 'kaggle':
        epochs_values = [3, 30, 50, 100, 150, 250]
        base_factors = 300
        base_lr = 0.01
        base_reg = 0.02
    else:
...
def menu_graphics(data):
    ...
    while True:
        ...
        if op == '1': plot_rmse_comparison(data)
            ...
            plot_rmse_vs_factors_svd(data)
            plot_rmse_vs_k_neighbors(data)
            plot_filter_bias(data)
            plot_rmse_comparison(data)
        elif op == '5': break
